...
const int MAX_FMAP_PACKED = 324; //max array size packed
const int MAX_F = 32; //{16, 32} num of conv2 output fmaps
const int MAX_W_CONV_UN = 4608;//3*3*16*32 num of conv2 weights
...
void pad(bitpack input[MAX_FMAP], bitpack output[MAX_FMAP], int M, int I);

// Reduce the size of feature map
void max_pool(bitpack input[MAX_FMAP], bitpack output[MAX_FMAP], int M, int I);

// Reshape the conv laper output to dense layer input
void reshape(bitpack* input, bitpack* output);

// Perform dense layer
void dense(bitpack* input, bitpack*output, const bit* weight, const float* bias, int M, int N, bool use_relu);

// Perform convolutional kernel
void conv(bitpack input[MAX_FMAP], bitpack output[MAX_FMAP], const bit8_t threshold[MAX_FMAP], int M, int N, int I, int L);
